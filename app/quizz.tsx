...
import { AnswerKey, Answers, ApiQuestions, QuizQuestion } from './types';
...
const QuizScreen = () => {
  ...
  const handleNextQuestion = () => {
    ...
    if (next < questions.length) {
      ...
      AsyncStorage.setItem(
        'lastQuizScore',
        JSON.stringify({
          //wordt nog opgehaald met getitem in dashboard_index.ts
          score,
    ...
    return (
      <View style={[styles.container, styles.center, { backgroundColor: colors.bg }]}>
        <Text style={{ color: colors.text, textAlign: 'center' }}>
          {error || 'Geen quizvragen gevonden.'}
        </Text>
        <Pressable
          style={[styles.retryButton, { backgroundColor: colors.fire }]}
  ...
  if (quizFinished) {
    let message =
      score < 5
        ? 'Je bent verloren! Probeer het opnieuw!'
        : score <= 7
          ? 'Goed gedaan! Voldoende.'
          : 'Fantastisch! Je bent een echte Avatar kenner!';
    return (
      <View style={[styles.container, styles.center, { backgroundColor: colors.bg }]}>
        <Text style={[styles.headerText, { color: colors.text }]}>Quiz Afgerond!</Text>
        <Text style={[styles.finalScore, { color: colors.fire }]}>
        ...
        <Pressable
          style={[
            styles.retryButton,
            { backgroundColor: colors.restartBtn, borderColor: '#b22222', borderWidth: 2 },
          ]}
          onPress={() => {
          ...
          <Text style={{ color: colors.restartText, fontWeight: 'bold' }}>Nieuwe Quiz Starten</Text>
        </Pressable>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View
          style={[styles.questionCard, { backgroundColor: colors.card, borderColor: colors.fire }]}>
        ...
        {(['a', 'b', 'c', 'd'] as AnswerKey[]).map((key) => {
          ...
          return (
            <Pressable
              ...
              <Text
                style={{
                  color: btnColor !== colors.defaultBtn ? colors.defaultBtn : '#333',
                  fontWeight: '600',
                }}>
        ...
        {answerSubmitted && (
          <Pressable
            ...
            <Text style={{ color: colors.nextText, fontWeight: 'bold' }}>Volgende Vraag ➤</Text>
          </Pressable>
        )}
      </ScrollView>
    </View>
