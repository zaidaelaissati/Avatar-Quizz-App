...
import { StyleSheet, Text, View, Pressable, Platform, ScrollView, Image } from 'react-native';
...
const DashboardScreen = () => {
  const { loading, logout } = useSupabase(); //gebruik supabase
  const { theme } = useContext(ThemeContext);//thema light darkmode
  const { name, avatar } = useUser();
//lokaal state voor scores bij te houden ,met asynsctorage
  const [lastScore, setLastScore] = useState<number | null>(null);
  const [maxScore, setMaxScore] = useState<number>(10); //default max score is 10
  const [lastQuizDate, setLastQuizDate] = useState<string | null>(null);

  const loadScore = async () => {
    try {
      const data = await AsyncStorage.getItem('lastQuizScore'); //werd geset in quizz.tsx, de data werd daar gestored
      if (data) {
        const parsed = JSON.parse(data); //parsen naar object
        setLastScore(parsed.score); //laatste score setten
        setMaxScore(parsed.maxScore); //maxscore setten  (max aantal punten dat mogelijk ws in de quiz)
        setLastQuizDate(new Date(parsed.date).toLocaleDateString()); //datum formatteren
      }
    } catch (error) {
...
//als subapase nog laadt, moet die de loading scherm tonen
  if (loading) {
    return (
  ...
  return (
    <View style={[styles.container, { backgroundColor: bgColor }]}>
      <ScrollView contentContainerStyle={{ alignItems: 'center', paddingBottom: 120 }}>
        {/* Welcome message */}
        <Text style={[styles.title, { color: textColor }]}>Welcome, {name}!</Text>
        ...
        <View
          ...
          <Text
            ...
            numberOfLines={1}
            adjustsFontSizeToFit={true}> {/*fontsize past zich aan */}
            {lastScore !== null 
              ? `${lastScore} / ${maxScore}`
        ...
        {/* Last Quiz Info */}
        {lastQuizDate && (
          <View style={[styles.infoBox, { backgroundColor: infoBg, borderLeftColor: infoBorder }]}>
            <Text style={[styles.infoText, { color: textColor }]}>Last quiz: {lastQuizDate}</Text>
          </View>
        )}
      ...
      {/* onderaan Navbar  ik heb custom navbars gebruikt*/}
