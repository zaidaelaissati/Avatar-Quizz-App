...
const SEASONS = ['1', '2', '3']; // lijst van seizoenen
//styling data per seizoen  en icoontjes
...
const EpisodesScreen = () => {
 ...
 //sorteer eerst op seizioen dan op episode nummer
      data.sort((a, b) => {
        ...
        return seasonA !== seasonB ? seasonA - seasonB : numA - numB; // checken of twee episodes uit verschillende seizoenen komen
        //als ze uit verschillende seiz komen dan sorteer op seizo,
        //zo niet dan sorteer op epidode in dat diezo : numA-numB
      });
  ...
  const getFilteredEpisodes = (): Episode[] => {
    ...
    if (selectedSeason !== 'All') list = list.filter((ep) => ep.Season === selectedSeason); //eerst filteren op geslecteerde siezoen
    //maakt zoekbalk lowercase, zorgt voor spatie weg 
    if (searchText.trim() !== '') { 
      const lower = searchText.toLowerCase();
      list = list.filter(
        (ep) =>
          ep.Title.toLowerCase().includes(lower) || //filtr op titel
          (ep.Description?.toLowerCase() || '').includes(lower) //filt op berschijcing
      );
    }
    return list;
  };
  ...
  const renderItem = ({ item }: { item: Episode }) => {
    ...
    return (
      <Pressable
        style={[
          styles.episodeCard,
          { backgroundColor: colors.card, borderColor: seasonData.color },
        ]}>
        ...
        <View style={styles.textContainer}>
          <Text
            ...
            ]}>{`S${item.Season} E${item.NumInSeason}: ${item.Title}`}</Text>
          <Text style={[styles.airDateText, { color: colors.text }]}>
            Aired: {item.OriginalAirDate}
          </Text>
          {item.Description && (
  ...
  const renderHeader = () => (
    <View>
      <Text style={[styles.headerText, { color: colors.text }]}>
        Avatar: De Legende van de Afleveringen
      </Text>
      <TextInput
        style={[
          styles.searchInput,
      ...
      <ScrollView
        ...
        contentContainerStyle={styles.seasonButtonsWrapper}>
            ...
            <Pressable
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <MaterialCommunityIcons
                  ...
                  style={{ marginRight: 5 }}
                />
                <Text style={{ color: isActive ? '#fff' : data.color, fontWeight: 'bold' }}>
                  {season === 'All' ? 'Alle Boeken' : `Boek ${season}`}
                </Text>
              </View>
            </Pressable>
          );
      ...
      {filteredEpisodes.length === 0 && !loading && (
        <Text style={{ color: colors.noResults, textAlign: 'center', marginTop: 20 }}>
  ...
  if (error)
    return (
      <View style={[styles.container, styles.center, { backgroundColor: colors.bg }]}>
        ...
        <Pressable
          style={[styles.retryButton, { backgroundColor: SEASON_DATA['2'].color }]}
